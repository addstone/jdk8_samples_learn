...
/**
 * Description:  Helper class to build a Netbeans project for a sample and open it in Netbeans
 * Description:  Helper类，用于为示例构建Netbeans项目并在Netbeans中打开它
 * Author: LuDaShi
 * Date: 2021-01-05-15:18
 * UpdateDate: 2021-01-05-15:18
 * FileName: SampleProjectBuilder
 * Version: 0.0.0.1
 * Since: 0.0.0.1
 */
public class SampleProjectBuilder {
    ...
    public static String loadAndConvertSampleCode(String urlToSampleJavaFile) {
        ...
        if (hasPlayMethod) {
            source = source.replaceAll("@Override public void play\\(\\)", "public void play()");
        }
        // remove @override from playMethod
        boolean hasContent3DMethod = findCreate3DContentMethod.matcher(source).find();
        if (hasContent3DMethod) {
            source = source.replaceAll("@Override public Node create3dContent\\(\\)", "public Node create3dContent()");
        }
